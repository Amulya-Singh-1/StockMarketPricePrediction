...
Original file is located at
    https://colab.research.google.com/drive/1zEMvN3O5Fsl_VmTkAAjTtrKBX2szjjhI
"""

# ### Data Collection
# import pandas_datareader as pdr
# key="46a4638a4167bff3ad6f0d27e7520f1902515227"

import pandas as pd

df=pd.read_csv('AAPLnew.csv', usecols=['Date','Close'], parse_dates=['Date'], index_col='Date')
...
plt.plot(day_new,scaler.inverse_transform(df1[182:]))
...
prev_values = df.iloc[:180] #train
y_test = df.iloc[180:] #test
...
df5 = pd.read_csv("RELIANCE.NS.csv")
df5.head()

df5.rename(columns={"Date":"date","Open":"open","High":"high","Low":"low","Close":"close"}, inplace= True)
df5.head()
...
print("Date column data type: ", type(df5['date'][0]))
print("Open column data type: ", type(df5['open'][0]))
print("Close column data type: ", type(df5['close'][0]))
print("High column data type: ", type(df5['high'][0]))
print("Low column data type: ", type(df5['low'][0]))
...
print("Starting date: ",df5.iloc[0][0])
print("Ending date: ", df5.iloc[-1][0])
print("Duration: ", df5.iloc[-1][0]-df5.iloc[0][0])

monthvise= df5.groupby(df5['date'].dt.strftime('%B'))[['open','close']].mean().sort_values(by='close')
monthvise.head()
...
df5.groupby(df5['date'].dt.strftime('%B'))['low'].min()

monthvise_high= df5.groupby(df5['date'].dt.strftime('%B'))['high'].max()
monthvise_low= df5.groupby(df5['date'].dt.strftime('%B'))['low'].min()
...
fig = px.line(df5, x=df5.date, y=[df5['open'], df5['close'], 
                                          df5['high'], df5['low']],
...
closedf = df5[['date','close']]
print("Shape of close dataframe: {0}".format(closedf.shape))
